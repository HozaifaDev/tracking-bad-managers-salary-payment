/**
 * Excel workbook export (ExcelJS): Summary, Monthly, Sessions, Payments.
 */
...
async function buildWorkbook({ from, to } = {}) {
  ...
  const summary = getAllTimeSummary(db);
  const monthly = getMonthlyBreakdown(db);

  let sessionSql = 'SELECT * FROM sessions WHERE 1=1';
  const params = [];
  if (from) {
    sessionSql += ' AND date >= ?';
    params.push(from);
  ...
  const payments = db.prepare('SELECT * FROM payments ORDER BY date ASC, id ASC').all();
  ...
  ws1.getCell('A4').value = 'Total Earned';
  ws1.getCell('B4').value = summary.totalEarned;
  ws1.getCell('B4').numFmt = EGP_FMT;
  ws1.getCell('A5').value = 'Total Paid';
  ws1.getCell('B5').value = summary.totalPaid;
  ws1.getCell('B5').numFmt = EGP_FMT;
  ws1.getCell('A6').value = 'Balance Owed';
  ws1.getCell('B6').value = summary.balance;
  ws1.getCell('B6').numFmt = EGP_FMT;
  const balCell = ws1.getCell('B6');
  if (summary.balance > 0) balCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFCCCC' } };
  else if (summary.balance < 0) balCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFCCCCFF' } };
  else balCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFCCFFCC' } };
